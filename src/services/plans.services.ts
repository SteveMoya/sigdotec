...
export const PlansService = {
    ...
    getPlansMini() {
        return Promise.resolve(this.getPlansData().slice(0, 5));
    },

    getPlansSmall() {
        return Promise.resolve(this.getPlansData().slice(0, 10));
    },

    getPlans() {
        return Promise.resolve(this.getPlansData());
    },

    getPlansWithOrdersSmall() {
        return Promise.resolve(this.getPlansWithOrdersData().slice(0, 10));
    },

    getPlansWithOrders() {
        return Promise.resolve(this.getPlansWithOrdersData());
    }
};

export const PlanService = {
    async getPlanUnit(id: number) {
        try {
            const unitPlan = await fetch(`${AI_URL}/unit_plan?topic_id=${id}`);
